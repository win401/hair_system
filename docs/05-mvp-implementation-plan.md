...
- [x] 링크 전용 접근 제한 (2026-09-16, Claude) — Vercel 비밀번호 보호는
  Pro 플랜 전용이라, `proxy.ts`(Next.js 16의 middleware 대체)로 직접
  구현했다. `SITE_ACCESS_TOKEN` 환경변수와 일치하는 `?token=` 쿼리 또는
  쿠키가 없으면 페이지는 `/invite-only`로 리다이렉트, `/api/*`는 401을
  반환한다 — `/api/generate` 직접 호출로 과금을 우회하는 경로까지 막는
  게 핵심이었다. 첫 방문 시 쿠키가 30일 유지된다. 로컬과 프로덕션 모두
  토큰 없음/토큰 포함/쿠키만 있는 경우를 직접 확인했다. `SITE_ACCESS_TOKEN`이
  없으면 게이트 자체가 비활성화되므로 기존 환경엔 영향 없음.
- [ ] M6 프라이버시·QA 마무리
